...
const EnhancedMobileParkDetails = () => {
    ...
    // Enhanced park data fetcher with better error handling
    useEffect(() => {
        const fetchPark = async () => {
            try {
                setLoading(true);
                console.log('Fetching park with slug:', slug);

                const q = query(collection(db, "parks"), where("slug", "==", slug));
                const snapshot = await getDocs(q);

                if (!snapshot.empty) {
                    const parkData = { id: snapshot.docs[0].id, ...snapshot.docs[0].data() };
                    console.log('Park data fetched:', parkData);
                    setPark(parkData);
                    setDataLoaded(true);
                    ...
                    console.log('No park found with slug:', slug);
                    setNotFound(true);
                }
            } catch (error) {
                console.error('Error fetching park:', error);
                setNotFound(true);
                showToast("Error loading park data", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    }, [slug, currentUser, showToast]);

    // Fetch reviews with proper parkId
    useEffect(() => {
        if (!park?.id) return;
        ...
        const fetchReviews = async () => {
            try {
                ...
                console.log('Fetching reviews for park ID:', park.id);

                const q = query(collection(db, "reviews"), where("parkId", "==", park.id));
                const snapshot = await getDocs(q);
    ...
    const handleReviewSubmit = async (e) => {
        ...
        if (!park?.id) {
            showToast("Park data not loaded", "error");
            return;
        }
    ...
    const handleLike = async (index) => {
        const likedKey = `liked-${park?.id}-${index}`;
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl overflow-hidden">
                    ...
                    <div className="p-4 md:p-6 lg:p-8 space-y-8">
                        ...
                        {/* Related Actions - Hidden on mobile, shown on desktop */}
                        {!isMobile && (
