  ...
  function App() {
    ...
    // Add this useEffect to your App.jsx component (after your existing useEffects):
  ...
  // Also add this error boundary for better error handling:
  useEffect(() => {
    const handleUnhandledRejection = (event) => {
      console.error('🔥 Unhandled Promise Rejection:', event.reason);
      
      // Show user-friendly error for FCM-related issues
      if (event.reason?.message?.includes('messaging') || 
          event.reason?.message?.includes('FCM') ||
          event.reason?.message?.includes('service worker')) {
        showToast('🔔 Push notification setup failed. You can still use the app normally.', 'warning');
      }
    };
