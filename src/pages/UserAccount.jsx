  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (userSnap.exists()) {
        ...
        const q = query(collection(db, "reviews"), where("author", "==", userData.displayName || currentUser.email));
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/80 backdrop-blur-md p-6 sm:p-10 rounded-3xl shadow-xl max-w-7xl mx-auto border border-white">
        ...
        {/* Favorites (Only show on desktop) */}
        <div className="hidden sm:block">
          <Section title="💖 Favorite Parks" loading={parksLoading} empty={favoriteParks.length === 0}>
...
const Section = ({ title, loading, empty, children }) => (
  <div className="mt-10">
    ...
    {loading ? <SkeletonLoader type="card" count={3} /> : empty ? <p className="text-gray-500">Nothing here yet.</p> : children}
  </div>
);
